...
function MasterCategoryPage({
    ...
    const onDestroy = React.useCallback(
        (data) => {
            Swal.fire({
                ...
                text: `Apa kamu yakin ingin menghapus Master Category ${data.name}?`,
                ...
                if (result.isConfirmed) {
                    toast
                        .promise(MasterCategoryService.Delete(data.id), {
                            pending: 'Menghapus data Master Category',
                            success: 'Data Master Category berhasil dihapus',
                        })
                        .catch((err) => {
                            toast(err?.message, {
                                type: 'error',
    ...
    const onBulkDestroy = React.useCallback((ids: any[]) => {
        Swal.fire({
            icon: 'warning',
            title: 'Hapus Beberapa Master Category',
            text: `Apa kamu yakin ingin menghapus beberapa Master Category?`,
            showCancelButton: true,
            confirmButtonText: 'Ya, Hapus',
            confirmButtonColor: 'var(--cui-primary)',
            cancelButtonColor: 'var(--cui-secondary)',
    ...
    const onExport = React.useCallback(() => {
        toast
        .promise(MasterCategoryService.Exports(filters), {
            pending: 'Export data Master Category',
            success: 'Data Master Category berhasil diexport',
        })
        .catch((err) => {
            toast(err?.message, {
            type: 'error',
